...
export default function EditUser() {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    await axios.put(`https://foodprophet-backend-production.up.railway.app//user/${id}`, user);
    navigate("/");
  };

  const loadUser = async () => {
    const result = await axios.get(`https://foodprophet-backend-production.up.railway.app//user/${id}`);
    setUser(result.data);
  };
